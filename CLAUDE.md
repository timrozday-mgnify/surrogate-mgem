...
>
> **Next — the second sweep, and conditioning is the open problem.** Everything
...
> | F `groupmax-u` seeded, width 128 × depth 3 | 3 | does depth help *once the pieces are seeded*? |
> | G `groupmax-u` seeded, width 1 × depth 1, K {100,1000} × T {0.03,0.1,0.3} | 6 | the accuracy-vs-conditioning frontier |
> | E the 4000-media set | 3 | the rows axis, wired to a real second label root |
>
> **`T` is the axis that matters and it is not an accuracy knob.** The seeded head
> reached 0.9733 cosine at T=0.03 with median Hessian condition **1.9e24** — the
> worst number in the whole session, and exactly what §8's Newton spends. Arm G's
> range is deliberately the *blunt* half (0.03-0.3) against what the untrained
> tangent model prefers (0.01-0.03), because conditioning binds before accuracy
> does, and because the untrained knee is a lower bound: fine-tuning can absorb the
> smoothing bias into the intercepts, which an untrained model cannot. Whether an
> affordable point exists on that frontier is **open**, and it is the single
> question that decides whether Head A is usable in §8 at all.
>
...
> lost its evidence, though its conditioning — 5.9e5 — is worth revisiting if Arm G
> comes back accurate but unaffordable); the `--gm-init random` control cells (the
...
> 0.973 (the one-organism result does not generalise), or Arm G finding no
> temperature that is both accurate and conditionable (Head A needs a different
> smoothing, or §8 needs a damped/trust-region Newton — P9's remedy — rather than a
> better head).
>
